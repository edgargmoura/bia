...
## Acceptance Criteria
- [x] `GET /api/versao` returns a JSON object with `versao`, `ambiente`, and `cliente` fields.
- [x] The Versão screen displays all three fields in separate cards.
- [x] `ambiente` is read from the env var `AMBIENTE` (fallback: `"development"`).
- [x] `cliente` is read from the env var `CLIENTE` (fallback: `"default"`).
- [x] Existing version behavior is preserved.
...
- [x] Create and switch to branch `005-feat-versao-screen-environment-and-client-info`
- [x] Update `api/controllers/versao.js` to return JSON with `versao`, `ambiente`, and `cliente`
- [x] Update `client/src/components/Versao.jsx` to parse JSON and render the two new cards
- [x] Manually verify the three cards render correctly in the browser
...
- [x] Confirm all checklist items above are marked as done
- [x] Review the full implementation (backend controller + frontend component)
- [x] Verify all Acceptance Criteria are met
- [x] If everything is correct, inform the user that task [005] has been completed
- [x] Move this file to `.kiro/tasks/done/`
- [x] Commit and push the moved file to the remote repository

> ✅ **PO Finalization — 2026-04-15:** Implementation verified and approved. All acceptance criteria met. Task moved to done.
